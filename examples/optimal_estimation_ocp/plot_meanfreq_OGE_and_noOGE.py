...
if __name__ == "__main__":
    ...
    MNF_EKF_biorbd_sum_noOGE = data['MNF']['EKF_biorbd']
    MNF_OGE_sum_noOGE = data['MNF']['OGE']
    MNF_OE_sum_noOGE = data['MNF']['OE']

    MNF_EKF_biorbd_mean_noOGE = np.mean(MNF_EKF_biorbd_sum_noOGE)
    MNF_EKF_biorbd_std_noOGE = np.std(MNF_EKF_biorbd_sum_noOGE)
    MNF_OGE_mean_noOGE = np.mean(MNF_OGE_sum_noOGE)
    MNF_OGE_std_noOGE = np.std(MNF_OGE_sum_noOGE)
    MNF_OE_mean_noOGE = np.mean(MNF_OE_sum_noOGE)
    MNF_OE_std_noOGE = np.std(MNF_OE_sum_noOGE)

    # Create dataset
    MNF = np.array(MNF_EKF_biorbd_sum + MNF_OGE_sum_noOGE + MNF_OGE_sum + MNF_OE_sum_noOGE + MNF_OE_sum)
    recons_type = np.array(['EKF'] * len(MNF_EKF_biorbd_sum) +
                           ['Joint angle tracking,\nnormal gravity'] * len(MNF_OGE_sum) + ['Joint angle tracking,\noptimized gravity'] * len(MNF_OGE_sum) +
                           ['Marker tracking,\nnormal gravity'] * len(MNF_OE_sum) + ['Marker tracking,\noptimized gravity'] * len(MNF_OE_sum))
    subjects = np.array([subject_trial[0] for subject_trial in subjects_trials] * 5)
    trials = np.array([subject_trial[1] for subject_trial in subjects_trials] * 5)
    number_shooting_points = np.array([subject_trial[2] for subject_trial in subjects_trials] * 5)
    ...
    save_path = 'Solutions/'
    fig.tight_layout
    save_name = save_path + 'MNF_controls_OGE_and_noOGE' + '.png'
